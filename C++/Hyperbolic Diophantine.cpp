    ...
    if ( p == 0 ) { //ad + bc = 0
        ...
        for ( int i = 1; i <= sqrtn; i++ ) {
            if ( p % i == 0 ) { //i is a divisor
                ...
                ///...
                /// Pairs Itselt..
                if (isValidSolution(a,b,c,p,i) && isValidSolution(a,b,c,p,-i ))
                {
                    cout << (i - c) / a << "," << (p - b*i) / (a * i) << endl ;
                    cout << (-i - c) / a << "," << (p - b*-i) / (a * -i) << endl ;
                }
                ///...
                if ( p / i != i ) { //Check whether p/i is different divisor than i
                    ...
                    if ( isValidSolution(a,b,c,p,-p/i) )res++;
                        /// ..
                        /// Pairs Itselt..
                if ( isValidSolution(a,b,c,p,p / i) && isValidSolution(a,b,c,p,-p / i ))
                        {
                            cout << (p / i - c) / a << "," << (p - b * (p / i)) / (a * (p / i)) << endl ;
                            cout << (-p / i - c) / a << "," << (p - b * (-p / i)) / (a * (-p / i)) << endl ;
                        }
                ///...
                }
...
int main()
{
    cout << hyperbolicDiophantine(2, 5, 56, -7) << endl ;
    return 0 ;
}
